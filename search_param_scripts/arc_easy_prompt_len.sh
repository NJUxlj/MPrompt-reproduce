...
step_log=$(( 15 * $gradient_accumulation_steps ))  # 原来的两倍 由于梯度累计
max_ques_length=512  # ques_opt 的长度
max_cont_length=512  # context 的长度
max_ans_length=50  # answer 的长度
...
kd_prompt_dropout=$prompt_dropout
# domain参数
# n_prompt_tokens=5
domain_lr=$lr
gap=5
...
# 需要变化的参数————prompt
prompt_lenght1='30 40 50 60' #   
prompt_lenght2='5 10 15 20 30 40 50 60' #   


for knowledge_sequence_length in $prompt_lenght1  #此处就不使用大括号了。
    do
        for n_prompt_tokens in $prompt_lenght2
            do
                $MY_PYTHON $EXEC \
                --epoch $EPOCH \
